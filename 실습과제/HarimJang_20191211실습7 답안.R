...
gmap + 
  geom_text( data = data.frame( gc ),
   ...
   # 차이 :    이름으로 먼저 벡터를 만들고 geocode에 대입!! 
   #           cen을 열의 평균으로 (하나로 묶에서) 만듬. 
...
setwd("d:/")
car <- read.csv("도로교통공단_시도_시군구별_월별_교통사고(2018).csv")    # 데이터 불러오기
car <- data.frame(car)                                                   # data.frame 만들기
car <- car[,-3]                                                          # 데이터 중 '월'열 제거 & 저장
car_sum <- car %>%  filter (시도 == '서울') %>% group_by(시군구) %>% summarise(sum(발생건수))   #'서울' 데이터 추출하고 시군구로 묶고 발생건수를 그룹별로 합계

city <- car %>% filter(시도 == '서울') %>% group_by(시군구)         # 시군구 이름만 추출하려고 다시 그룹으로 묶고 다른 변수에 저장
city_names <- as.vector(city$시군구)                                # 벡터로 만듬
city_names <- unique(city_names)                                    # ? 왜했지..?
city_names <- as.vector(city_names)                                 #또... 벡터만듬/// ㅠㅠ - 이름을 추출하려고 .....

gc.car <- geocode(enc2utf8(city_names))           # 각 도시의 위치의 경도 위도 추출
df.car <- data.frame(name = city_names, lon = gc.car$lon, lat = gc.car$lat)   # 도시 이름과 경도, 위도 를 데이터 프레임으로 만듬
car_sum <- as.data.frame(car_sum)                                   # 발생건수를 만든걸 데이터 프레임으로 만듬
colnames(car_sum) <- c("name", "sum")                               # 각 열에 이름 지정
df.car <- left_join(df.car, car_sum, by = 'name')                   # 두 데이터를 합침

cen.car <- c(mean(df.car$lon), mean(df.car$lat))                    # 위도 경도 평균값 (중간값)

map.car <- get_googlemap(center = cen.car, maptype = "roadmap", zoom = 11, markers = gc.car)           # 지도 그리기
gmap.car <- ggmap(map.car)
gmap.car + 
  geom_point(data = df.car, aes(x = lon, y = lat, size = sum, alpha = 0.3), col = "blue") +            #버블 만들기
...
gmap +
  ...
  scale_size_continuous( range = c(1, 14 ) )
          ...
          # ? : aggregate, by = list ,subset, ,,,등 <-  코드 활용, 작용 다시보기. 
...
gmap.j + 
  ...
  scale_size_continuous(range = c(8,15)) 
    ...
    # *****왜 (내거 숙제)  제주 할때 서귀포랑 제주시가 같은 값이 두개씩 나와서 데이터 프레임이 만들어 졌는지 .. 다시 확인하기 **********

...
idx <- c()
for ( i in 1:nrow( ds ) ) {
  if ( sum( is.na( ds[ i, ] ) ) > 0 ) {     #행위치 지정 : ds[ i , ]
    idx <- c( idx, i )                      #결측지가 포함된 행의 데이터이니까 빈 벡터 idx를 만들고 인덱스 지정  
  } 
}
ds[ idx, ]
...
Oz <- AQ[,'Ozone'];  Oz
a <- mean(Oz,na.rm = T)
AQ$Ozone[AQ$Ozone %in% NA] <- a

So <- AQ[,'Solar.R']; So
b <- mean(So, na.rm = T); b
AQ$Solar.R[AQ$Solar.R %in% NA] <- b

AQ2 <- data.frame(Ozone = AQ$Ozone, Solar.R = AQ$Solar.R)
AQ2
...
sta[order(sta$Income, decreasing = T),]
...
library(dplyr)
Ill <- head(sta[order(sta$Illiteracy), ], 10)
Ill %>% select(Illiteracy)
